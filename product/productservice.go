...
func (ps *ProductService) Inject(logger flamingo.Logger,
	c *struct {
		CurrencyCode            string   `inject:"config:commerce.product.customservice.currency,optional"`
		DeliveryDefaultProducts bool     `inject:"config:commerce.product.customservice.defaultProducts,optional"`
		DeliveryCodes           []string `inject:"config:commerce.product.customservice.deliveryCodes,optional"`
		TestDataFolder          string   `inject:"config:commerce.product.customerservice.jsonTestDataFolder,optional"`
	},
) *ProductService {
	ps.logger = logger
	if c != nil {
...
// Get implements the ProductService and returns a product struct
