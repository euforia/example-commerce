...
func (s *SearchService) Inject(
	productService *ProductService,
	cfg *struct {
		LiveSearchJSON         string       `inject:"config:commerce.product.customservice.jsonTestDataLiveSearch,optional"`
		CategoryFacetItemsJSON string       `inject:"config:commerce.product.customservice.jsonTestDataCategoryFacetItems,optional"`
		SortConfig             config.Slice `inject:"config:commerce.product.customservice.sorting"`
	},
) *SearchService {
